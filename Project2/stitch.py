...
def stitching(img_path, offsets):
    ...
    x_boundary = 0
    y_boundary_max = 0
    y_boundary_min = float("inf")
    for offset in offsets:
        x_boundary += offset[0]
        if offset[1] < y_boundary_min and offset[1] >= 0: 
            y_boundary_min = offset[1]
        if offset[1] < 0: 
            y_boundary_max += abs(offset[1])

    result_image = np.zeros((dst_h+abs(y_boundary_min)+abs(y_boundary_max), dst_w+abs(x_boundary), 3))
    result_image[abs(y_boundary_min): abs(y_boundary_min)+dst_h, :dst_w] = img_list[0]
    ...
    for i, (img1, img2) in enumerate(zip(img_list[:], img_list[1:])):
        ...
        result_image[abs(y_boundary_min)-prev_offset[1]:abs(y_boundary_min)-prev_offset[1]+prev_h, abs(final_offset[0]):abs(final_offset[0])+overlap,:] = prev_overlap
        result_image[abs(y_boundary_min)-final_offset[1]:abs(y_boundary_min)-final_offset[1]+cur_h, abs(final_offset[0]):abs(final_offset[0])+cur_w, :] += cur_pixels
