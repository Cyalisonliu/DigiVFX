...
def draw_keypoint(img, raw_keypoints, contrast_keypoints, curve_keypoints):
    ...
    ax.set_title('Keypoints after removing low contrast extrema')
    plt.show()
    ...
    ax.set_title('Keypoints after removing edge points using principal curvature filtering')
    plt.show()

def draw_final_kpts(img, kpts):
    # Plot keypoints on image
    fig, ax = plt.subplots()
    ax.imshow(img, cmap='gray')
    ax.plot(kpts[:, 0], kpts[:, 1], 'y+')
    ax.set_title('DOG extrema')
    plt.show()
